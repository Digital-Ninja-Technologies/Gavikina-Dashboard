import { naira } from './data.js';

export function csvFor(list) {
  ...
  list.forEach((l) => {
    lines.push([l.type, l.name, l.phone, l.email, l.contact, l.area, l.role, l.cv, l.property, l.reason, l.backup,
      ...
      l.message || l.about || l.reason || l.ai || '', l.when].map(cell).join(','));
  });
  return lines.join('\n');
}
...
export function download(name, text) {
  try {
    ...
    /* clipboard/download blocked — nothing else to do in the prototype */
  }
}

export function summaryOf(l) {
...
export function tagColors(type) {
  return TAG_COLORS[type] || ['rgba(20,55,94,.6)', 'rgba(20,55,94,.07)'];
}
